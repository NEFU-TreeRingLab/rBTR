...
#'
#'
compute_clim <- function(climdata , start.year = NA, end.year = NA ,
                         lat = 44.13, rootd = 1000,... ){
  ## Column Name
  .dtColname <- colnames(climdata) <- toupper(colnames(climdata) )

   #### error-catching ####
  if (lat>90 | lat < -90) {
    stop('Latitude should be between ±90!')
  }

  if (is.na(start.year) ) {start.year <- min(climdata$YEAR) } else {
    if( start.year > max(climdata$YEAR) ){
      stop("start.year is wrong")}
  }

  if (is.na(end.year)) {end.year <- max(climdata$YEAR)} else {
    if( end.year < min(climdata$YEAR) ){
      stop("start.year is wrong")
    }
  }
  #### 检查气象数据，看是否需要通过计算补齐 ####
  ## check date col
  .checkCol <- 0
  if ("YEAR" %in%  .dtColname) {
    if ('DOY' %in% .dtColname) {
      climdata <- dplyr::rename( climdata , Year = YEAR )
      .checkCol <-2
    } else {
      if ( all( c('MONTH', 'DAY') %in% .dtColname) ) {
        climdata <- dplyr::rename(climdata,Year = YEAR,Month = MONTH,Day = DAY  )
        .checkCol <- 1
      } ## M & D
    }## DOY
  } ## YEAR
  if( .checkCol == 0 ){stop("The date should be in the format of 'Year' 'Month' 'Day' or 'Year' 'DOY'.")}

  ## check Climfactor
  .checkTem <- .checkVPD <- .checkSoilM <- .checkLs <- .checkRootd <- 0

  .checkTem[ 'TEM' %in% .dtColname ] <- 2

  .checkVPD[ 'RH' %in% .dtColname ] <- 1
  .checkVPD[ 'VPD' %in% .dtColname ] <- 2

  .checkSoilM['PRE' %in% .dtColname] <- 1
  .checkSoilM['SOILM' %in% .dtColname] <- 2
  if ( .checkSoilM ==2 ) {
    climdata <- dplyr::rename(climdata,soilM = SOILM )
  }
  ...
  ## 提取对应年份数据
  climdata <- dplyr::filter( climdata,Year %in% c( start.year:end.year) )
