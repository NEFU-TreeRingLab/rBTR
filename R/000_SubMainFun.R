...
#' Submain function calculate tree-ring anatomic
#'
...
btr_lite <- function(  clim, parameters, syear = NA, eyear = NA , mCRD = 16){ ## functions start  ring_width,
  ...
  fixparam.growth.vessel <- filter(parameters ,  modul == "growthV" & paramtype == "fixed") %>%
    dplyr::select( c("parameter","values") ) %>% tidyr::spread( key = parameter, value = values)
  ...
  # dailyParameters1 = matrix(NA,ncol = 15,nrow = 366)
  # colnames(dailyParameters1) <-c("years","Today",colnames(dynparam.growth.t))
  ##
  ...
  for (years in syear:eyear) { ## year cycle start  年循环计算
    ...
    gR.year = filter(microclim , Year == years)
    ...
    dynparam.growth.t$T_age <- dynparam.growth.t$T_age + years - syear
    ...
    for (Today in 1:nrow(gR.year)) { ## today sycle start  日循环 nrow(gR.year) ## nrow(gR.year)
      ...
      if (actAccumulatedTem >= fixparam.divi$AT & clim.today$rootd > 0 ) { ## if start 可以生长时计算细胞分裂分化
        ...
        growthing.cells = filter(xylogenesis[["cells"]] , DDOY ==0  )
        if (layer.max != 0 & nrow(growthing.cells) != 0) {



          # growthing.cells = cells_growth(cell = growthing.cells, CorV = "C" , clim.today, layer.max, P_gR,
          #                   fixparam.growth.fiber,fixparam.growth.vessel, dynparam.growth.t,redir)

          growthing.cells$CRD  = mCRD
        ...
        if(layer.max != 0 & nrow(growthing.cells) != 0  ){  ## 有导管时导管生长
          ...
          # growthing.cells = cells_growth(cell = growthing.cells, CorV = "V" , clim.today, layer.max, P_gR,
          #                   fixparam.growth.fiber,fixparam.growth.vessel, dynparam.growth.t,redir)
          growthing.cells$CRD  = mCRD
          growthing.cells$DDOY = clim.today$DOY
